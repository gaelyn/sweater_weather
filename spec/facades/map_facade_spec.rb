...
RSpec.describe 'Map Facade' do
  ...
  it 'accounts for edge case of very long trip', :vcr do
    route = MapFacade.get_road_trip("Anchorage,AK", "Denver,CO")
    expect(route).to be_a(Route)
    expect(route.weather_at_eta).to eq("Forecast not available")
  end
end
